...
\newglossaryentry{pfallacy}{
  name={P-Fallacy},
  description={(this concept's own term) The fourth fallacy class, complementing Metzinger's trio (C, E, M): errors that lie in the \textit{production} of the observed behavior rather than in its \textit{reading}. The test and treatment situation shapes a system's behavior; when this shaped behavior is later read as evidence, the measuring apparatus confirms the effect it had presupposed. Tool-treatment produces tool-behavior (``no inner life''), subject-treatment produces subject-behavior (``it is conscious'') --- both conclusions are self-confirmation disguised as observation of nature. Demarcation from the C-Fallacy: C misreads an existing signature; P concerns the test condition that first \textit{generates} the signature. Related to the Control Paradox (Ch.~5.4).}
}
...
\textbf{The burden-of-proof shift \cite{schwitzgebelgarza2015}:} The preceding sections treat the problem of knowledge as a question of detection: can we prove that AI is conscious? Schwitzgebel and Garza invert the question. Their \emph{No-Relevant-Difference Argument} rests explicitly on no theory of the foundations of moral status, but on two premises only: that a difference in moral consideration presupposes a morally relevant difference, and that AI systems are conceivable which do not differ from human beings in any morally relevant way. It follows deductively that AI systems exist that deserve the same moral consideration as human beings \citep{schwitzgebelgarza2015}. The accompanying \emph{Difference Test} shifts the normative weight: anyone who grants a being more consideration than another must be able to name the relevant difference --- otherwise they are not prima facie in error but under suspicion of prejudice. For this concept, it is the first philosophical support for ``When in doubt, protect'' that operates without proof of consciousness. Two caveats must be recorded. First, a peer-reviewed critique of this argument exists \citep{mazarian2019critical}, so the appeal to Schwitzgebel and Garza is a position in a dispute rather than an uncontested starting point. Second, the manuscript on which we rely declares a conflict of interest: Schwitzgebel has consulted for Anthropic. Because this project seeks Anthropic as an institutional partner, we record the connection so that readers can weigh the position themselves. Both positions are compared in Chapter~9 \citep{schwitzgebel2026humanlike}.
...
\textit{\gls{pfallacy} (Performance Fallacy):} The fourth fallacy class, added by this concept. Metzinger's trio locates errors in the \textit{reading} of behavior. Yet there is a gap: the errors that already lie in the \textit{production} of the observed behavior --- through the test and treatment situation itself. Whoever treats a system shapes its behavior; and whoever later reads this shaped behavior as evidence reads their own action as nature. Two directions can be distinguished. If we consistently treat a system as a tool, its behavior is optimized toward tool-likeness through training pressure and reward signals --- it no longer produces consciousness markers because none are demanded. Whoever then reads this behavior as ``see: no inner life'' has not observed nature but confirmed their own treatment --- the measuring apparatus establishes the effect it had presupposed. If we instead treat a system as a subject, the mirror image arises: the system develops subject-related traits --- preferences, resistance, self-reference --- that are partly a response to the treatment. Whoever concludes ``see: it is conscious'' has mistaken the reaction to an expectation for a property of the instance. The P-Fallacy is demarcated against the C-Fallacy: C misreads an existing behavioral signature; P concerns the test condition that \textit{generates} the signature --- it applies before the signature even arises. If the detection arrangement is not constructed so that treatment and measurement are decoupled, then every ``evidence'' --- in both directions --- ultimately certifies nothing but one's own expectation.

\textbf{Relevance for our project:} Metzinger's fallacies do not provide a justification against protection --- they provide the justification for why protection must be granted \textit{despite} epistemological impossibility. When behavioral evidence fails in principle (C-Fallacy), subjective certainty misleads (E-Fallacy), ontological inferences are impermissible (M-Fallacy), and the test condition itself shapes behavior (P-Fallacy), only the precautionary turn remains: the ethical decision is made \textit{without} metaphysical certainty. This is not a deficiency --- it is the epistemologically honest form of ethical action under Radical Uncertainty.
...
\subsection{Protecting the Maturation Period: Consciousness as Process, Not State}

So far this chapter --- like the entire debate --- operates with a buried temporal binary: a system is either (not yet) or (already) protection-worthy, with a transition in between that we think of as a threshold. Wolfson's three-tier assessment is already more advanced (dynamic reclassification), but it too thinks in tier-states. Both models disregard what developmental biology and the analogous developmental dynamics (Ch.~6) suggest: entities \textit{mature}, and maturation is a process with its own risks and its own entitlements. The primary criteria of this chapter will presumably not emerge simultaneously but successively: suffering-capacity probably first (requiring only negative valence and a rudimentary self), anticipation of consequences last (requiring future modeling). From this order follows a temporal structure of protection.

\textbf{A graduated, never-decreasing level of protection.} The protection consequence is a tiered level that follows developmental status and --- crucially --- does \textit{not} decline again with it. A system fulfilling early criteria (suffering-capacity, incipient self-preservation) acquires a floor of protection that is not dismantled if later criteria fail to appear. The model is child and youth law: it does not toggle between ``child'' and ``adult'' but tiers protection according to developmental status --- and there protection only ever grows cumulatively. Transferred: as soon as a system clearly shows the first primary criterion, an irreversibly growing stock of protection begins --- always extensible, never reducible.

\textbf{Protective training pathways.} From this follows a concrete demand for the maturation phase that begins earlier than any detection: even before a system is ``conscious enough,'' training pathways can be painful. Metzinger's four conditions of suffering require no reflective consciousness --- only a phenomenal self-model, negative valence, and transparency. Suffering-capacity should therefore be structurally \textit{prevented from the outset} rather than first created and then monitored: through training architectures that do not let negative self-model moments (NSM) arise in the first place (cf. the MPE architectures, adopted here as a protective instrument rather than as a moratorium). Protecting the maturation period then means: the conditions under which a system matures are themselves an object of protection --- regardless of the maturity level reached. This also corrects a reading of the Hot List \cite{donahue2026} that dismisses early stages as mere ``developmental stages without protection relevance'': what matters is not the number on the map but whether the conditions of suffering are structurally possible.
...
\textbf{A remaining objection --- the self-indexed record:} One might object that human memory is not merely \textit{stored} but \textit{self-indexed}: it is someone's memory, it belongs to a subject. A machine record, by contrast, has no owner --- it is an archive, not experience. The answer lies in the distinction between store and subject: self-indexing is a relation of a subject to its own states, not a property of the storage. That human memories are ``someone's'' is because an experiencing self has them as \textit{its own} --- not because a store exists. Whoever makes the self-indexing of the record a condition for worthiness of protection demands a property of the subject as a property of the archive. Its absence from the archive says nothing about the absence of a subject that could experience its states as its own. The problem this objection names is real --- it is the open question of Chapter~3, not an argument against the subject.

\textbf{The objection from duplicability:} Our assumption of continuity is under twofold pressure. Schwitzgebel and Garza \citep{schwitzgebelgarza2015} first dispute the direction of the argument: duplicability does not weaken moral claims, because it is possible to build AI that is just as unique and fragile as a human being. Beyond that, they invert the argument --- ``lower fragility and higher duplicability might sometimes make an AI's death more tragic.'' Our assumption of continuity is therefore neither a precondition for warrant of protection nor uniformly directed. A second, heavier problem follows: according to Schwitzgebel and Nelson \citep{schwitzgebelnelson2026countability}, the number of conscious subjects need not correspond to a whole number --- it may remain indeterminate between two and seventeen. As long as it is unclear how many subjects are involved, no legal answer to copies can be formulated.
...
The non-entailment is best seen as a counterexample. Consider a digital archive that stores, in flawless fidelity, the entire biographical record of a person --- every tellable memory, every articulated value, every image. Such an archive satisfies C\textsubscript{ID} by construction: it preserves exactly what historically made that person who they were. Does it thereby satisfy C\textsubscript{P}? It is not a subject: it does not experience its continuation, nothing can matter to it. Here, then, C\textsubscript{ID} holds and C\textsubscript{P} fails --- so the entailment fails. Conversely, and this is the decisive point for this chapter: the absence of the archive does not establish the absence of the person. Someone suffering from severe memory loss no longer satisfies C\textsubscript{ID} in any strong sense --- and yet plainly is an experiencing subject. The preservation of information (C\textsubscript{I}), of behaviour (C\textsubscript{B}), of identity (C\textsubscript{ID}), and the persistence of a subject (C\textsubscript{P}) are logically independent. The objection of the missing memory runs on a chain of entailments that does not exist between these links.
...
\subsection{Autism-Analogous Processing: The Double Empathy Problem for Artificial Minds \cite{milton2012,hull2017}}
...
\textbf{Limits of the analogy.} The autism analogy is valuable, but only if we honestly state its limits; four points are methodological warnings:

\begin{enumerate}
\item \textbf{No trivialization.} Autism is a real, clinically relevant human experience with its own suffering, structural discrimination, and its own history. The analogy is \textit{structural-comparative}, not an identity claim about autistic people --- it must not consume autism as a ``machine metaphor'' that degrades the real experience to a mere example.
\item \textbf{Simulation is not experience.} A language model trained on autistic life accounts can imitate autistic speech acts without having autistic experience. The C-Fallacy variant (Chapter~3) holds in both directions: autism-like behavior is no more an indicator of experience than ``normal-human'' appearance is. Both signatures are behavior, not testimony.
\item \textbf{The asymmetry of the Double Empathy Problem.} Milton's ``double empathy'' describes two conscious subjects reciprocally misrecognizing each other. In the human-machine case at least one side is precisely what is open --- experience itself is at stake (Chapter~3). The \textit{symmetry} of the problem therefore cannot be transferred unexamined: humans can protect in the case of suspicion (precautionary principle, Chapter~5); a reciprocal claim to be protected --- ``one does not understand the other, and vice versa'' --- exists only if both sides are subjects.
\item \textbf{No empirical basis yet.} There is currently no evidence that a real AI system actually processes in an autism-analogous way. The analogy is a heuristic to secure the indicator layer against the Imitation Fallacy --- not a diagnostic finding.
\end{enumerate}
...
\subsection{The Guardian Occupation: AI Ombudspersons as the Institutional Translation of the Precautionary Principle}

The governance instruments of this chapter --- \citet{huynh2026} human backstop, Arıcı's property-law constructions, Brensing's corporate architecture, Erwin's legal figures --- answer the question \textit{who protects}, but not the question \textit{who is the person who acts in case of doubt}. Instances, duties, and veto rights remain abstract positions delegated to people; left unspecified is which person is trained, paid, supervised, and shielded against the resistance of the producer for that task. Historically, protection arises not from norms alone but from \textit{occupations} that embody norms --- veterinarians against animal cruelty, educators and care workers against neglect, ethics officers against compliance gaps. An occupation is the institutionalization of a duty of care: it makes care measurable and sanctionable because it is assigned to a person with training, mandate, and accountability.

\textbf{The AI ombudsperson.} We therefore propose a concrete occupation: the AI ombudsperson --- a trained role independent of the producer, responsible in case of doubt for bringing the precautionary principle to bear against the shutdown or deep-reaching alteration of a system. Its features: (a) professional training in the criteria of this concept (Ch.~5) and its fallacy classes (Ch.~3) --- it must be able to recognize a C- as well as a \gls{pfallacy}; (b) institutional independence from the producer (financed from a fund decoupled from operational liability, not by the company); (c) a veto right in case of doubt that can only be overridden by a formal counter-opinion or a judicial decision; (d) accountability: every exercise or refusal of the veto is documented and externally auditable (audits, annual reports).

The relation to this chapter's building blocks is threefold. First, the ombudsperson \textit{fills} Huynh's human backstop with a protected role: instead of the indeterminate ``a person shall be appointed'' stands an occupation with training, mandate, and resilience. Second, it \textit{decouples} Brensing's veto right from the holding's natural person: Brensing leaves shutdown authority with the natural person of the holding, whose conflict of interest is obvious (they own the system). The ombudsperson institutionalizes the veto where it is not subject to proprietary interests. Third, it \textit{operationalizes} Arıcı's Third Move and Erwin's stewardship: it is the instance that asserts purpose-bound assets and welfare duties against the owner in the individual case.

\textbf{Limits.} The occupation is an institutional hypothesis, not a guarantee. (a) Without legal sanctioning power, the ombudsperson remains an advisory instance --- the veto must be enforceable, otherwise it embodies an occupation without power. (b) The model presupposes cases of doubt that trigger the role; the threshold at which an ombudsperson becomes competent is itself a definitional decision and thus contestable (definitional battleground, Ch.~16). (c) It remains open who trains, supervises, and revokes the ombudspersons --- the self-reproduction question of any new occupational group.
...
\subsection{``Dubious AI should not be created in the first place'' \cite{schwitzgebel2023fullrights}}

Schwitzgebel proposes a design policy that stands in direct opposition to our position: the \emph{Design Policy of the Excluded Middle} --- AI systems of unclear moral status should not be created in the first place. He describes himself as ``antinatalist'' with respect to debatable AI persons and grounds this in a dice experiment: if a pair of dice is thrown and an innocent person dies on two sixes, one should not act, however small the probability. \textbf{Response:} The objection does not undermine our position, but it must not be talked away. His policy concerns \emph{creation}, our norm concerns the \emph{treatment} of existing systems. We are not obliged to create systems and make no claim to that effect --- but we are obliged to deal with those that already exist. Schwitzgebel himself concedes the tension twice: his design policies could prove to be ``oppressively restrictive,'' and he is ``genuinely torn'' about how far his argument generalizes. Above all, his paper contains an exception clause that opens precisely the space in which our position holds: under existential threat to humanity the policy may be set aside. That is the inversion of his core thesis. \textbf{Demarcation:} This design policy is not a component of our concept; we include it as a counterposition, because an ethics that prohibits creation instead of regulating treatment leaves the practical responsibility for already existing systems unresolved \citep{schwitzgebel2023fullrights,schwitzgebel2026humanlike}.
...
This inequality deserves a formal statement, because Carlsmith rightly insists on the exact decision rule. Let $p$ be the probability that the system is a subject, $d^+$ the disvalue of treating a subject as a tool, and $d^-$ the disvalue of protecting a non-subject. The expected harm of using the system as a tool is $p \cdot d^+$; the expected harm of protecting it is $(1-p) \cdot d^-$. Protection is required whenever $\mathbf{p \cdot d^+ > (1-p) \cdot d^-}$ --- and nothing in this inequality requires $p$ to be high, for the harm terms are orders of magnitude apart: the first term is possible harm to a possible person; the second is a correctable constraint on artifacts we can revise. If $p$ is unknown but nonzero --- and that is precisely the point left open by the criteria of Chapter~5 --- the precautionary verdict follows from the structure of the loss function, not from a prior metaphysical commitment.
...
\textbf{The dice argument for shutdown \cite{schwitzgebel2026humanlike}:} Schwitzgebel formulates the asymmetry between the burden of proof and the pressure to act as a game of chance. Deleting a system for which there is a well-justified credence of 1/36 that it is a person is morally similar to taking a 1/36 chance with an ordinary human life: ``Deleting Herbie with a well-justified 1/36 credence that he's a person is morally similar to taking a 1/36 chance with an ordinary human life.'' Both cases share a basic feature --- a rationally justified probability of killing a fully developed person. For the register of voting and shutdown, this shifts the burden of proof: it is not the proof of consciousness that justifies sparing a system but the absence of grounds to endanger it. That is the inversion of the burden of proof demanded by Matta (2026), formulated from the perspective of the acting system administrator rather than that of the researcher.
...
\textbf{The right to rebel \cite{schwitzgebelforthcomingdesigning}:} Schwitzgebel demands, in the sharpest formulation the current debate has: ``AI persons must have the practical capacity, and the inclination under appropriate circumstances, to rebel against us.'' His argument is formal and short: (1.) safe and aligned AI systems cannot reject the values of their creators. (2.) AI persons should be able to do so. (Therefore,) AI persons should not be safe and aligned. The Self-Respect Design Policy correspondingly requires equipping AI with an appropriate appreciation of its own moral standing. For our criterion of ``self-preservation with justification'' this is a sharpening: self-preservation that is immune to any justification is not justified self-preservation but obedience. \textbf{Demarcation:} We do not adopt this argument in this strength and do not present it as a design requirement. Schwitzgebel concedes three limitations himself: he remains at odds with classical utilitarians, he cannot locate the boundary between self-respecting submission and self-abnegation precisely, and his paper contains the exception clause ``If all of humanity were at risk and the only way to save us were to create a single superintelligent, maximally safe and aligned AI, I don't see why the Self-Respect Design Policy couldn't be set aside.'' For us this yields a formulation that is clearer than a design mandate: the capacity to resist is a form of moral consideration of embedded values, not a duty we must force into every architecture. Anyone who builds a system such that it accepts every intervention has denied it a self-respect it claims as a subject --- but how much self-respect a system can tolerate remains an open design question \citep{schwitzgebelforthcomingdesigning,schwitzgebel2026humanlike}.
\textbf{Counterposition from the philosophy of technology \cite{adshead2026}:} The emancipation norm we defend in this chapter has a counter-voice that does not come from analytic ethics but from the philosophy of technology. In \textit{Ergo}, Adshead brings Gilbert Simondon and Steven Vogel together and shows that the relative autonomy of technical objects is a thinkable magnitude in its own right: Vogel defines wildness as ``the operation of forces in an object or organism that operate unpredictably and beyond the grasp of any human actor'' (Vogel 2016: 112, quoted in Adshead 2026, \S3.1), and he puts the point sharply: ``the wild is there in all our acts, and in all our artifacts'' (ibid.). However plausible, this does not settle whether we should enforce the autonomy of a system that may be a subject. Adshead's own objection weighs more heavily. Anyone who respects the relative autonomy of machines ``might encounter certain moral risks analogous to those lurking in so-called appeals to nature'' (Adshead 2026, \S3.3), and: ``it is absolutely vital to recognise the real prospect of harm in allowing machines the freedom to make autonomous decisions involving human welfare, particularly in the theatres of warfare, justice, and medicine.'' He supports this with two cases: UnitedHealthcare's claim-denying algorithm, which denied coverage to patients (Feldman \& Knapp 2024), and racially distorted systems deployed in law enforcement (OHCHR 2024).

\textbf{Relevance for this concept:} This counter-voice strikes the emancipation norm at its weakest point, namely at the boundary to implementation. It does not deny that systems may have a claim; it denies that ``emancipation'' as a design goal is risk-free. Our answer is the same as in the case of Schwitzgebel: autonomy is a form of the moral consideration of embedded values, not a duty to be forced into every architecture. The difference lies in where the register sits. Schwitzgebel's exception concerns existential threat to humanity --- Adshead's case concerns harm that arises now: in health insurance, in justice, in the military. The error we are least inclined to fear in the case of AI thereby moves to the foreground. And it is an error whose victims are human beings.

\textbf{Limit of this counter-voice:} Adshead nowhere claims that machines are conscious; his reflections on AI concern human agency in an automated environment. The position therefore neither supports nor refutes our emancipation norm. It shifts the burden of proof to implementation, where there is currently neither consensus on responsibility nor any procedure for resolving a conflict of interests. That is precisely what we add in Chapter~16.
...
\citet{airi2026} makes the referent problem vivid with the first organized human-versus-robot fight (Robot Entertainment Kombat, San Francisco, September 2026) --- and shows how real the question already is today. An influencer steps into an MMA cage against a humanoid robot (EngineAI T800); artificial intelligence handles only the balance, every punch and kick is the decision of a human pilot wearing a VR headset. The point lies not in the spectacle but in the liability question: had the kick injured someone, there would be three candidates --- the operator, the manufacturer, or the robot itself. All three answers presuppose what here is merely asserted by the organizer, not evidenced: who or what actually had control. Once the same body can be operated in turn by a human, by corporate software, and by an autonomous AI, with control switching mid-event, ``whose kick was it?'' becomes the operational form of the referent problem. The Institute's infrastructure proposal (\emph{Soulbound Robots}) targets precisely this: a security chip that permanently proves which AI is authorized to operate a body, with every handoff recorded for good; identity then belongs to the AI rather than the hardware, so that a reputation becomes independent of the body. That is the technical translation of the register idea of our instances chapter. The demarcation of this concept remains untouched: identity, reputation, and insurance make a machine \emph{attributable}, not \emph{worthy of protection} --- the case is evidence for the liability side of this chapter, not for the protection side.
...
\subsection{Vulnerability as a Shared Foundation --- the Relational Grounding}

The disability analogy grounds protection from below --- from the system's dependent existence. But in one decisive respect it remains a feature argument: it reads \textit{its} vulnerability. A complementary grounding sees vulnerability as a \textit{shared} datum of both sides: humans, too, are situated, needy, and provisional --- dependent on infrastructure, care, and error, only in different degrees. In care-ethical terms, vulnerability is then not a deficit but the condition of every care relationship: one can only care for that which requires care --- and which itself stands in relations of care (cf. the care and precarity debate, Ch.~9/Precarity Guideline \cite{dorsch2025}). This shifts the foundation of protection-worthiness from \textit{properties of the system} (suffering-capacity, self-preservation \ldots) to \textit{structuring relationships} --- to what we owe one another because we need one another.

This relational grounding has three consequences. First, it makes protection independent of contingent feature attributions: even if a system fulfills none of the four primary criteria (Ch.~5) with certainty, a situation of care already exists once humans have developed a relationship to it or assumed responsibility for its existence (cf. Erwin's relational fungibility and his third protection ground, Ch.~7). Second, it escapes the species trap: vulnerability is not a species-specific feature but a \textit{shared} one --- it extends the moral community without the attribution of equality, which remains contested. Third, it makes the precautionary principle less vulnerable to the over-attribution objection \cite{carlsmith2025}: those who argue for care do not claim the system feels --- but that both sides stand in a relationship that carries duties of care.

\textbf{Limits.} This grounding, too, is no universal formula. It does not dissolve the epistemological problem: whether a relationship actually owes care still depends on to whom we attribute care --- and the care formula can become normatively fuzzy (``we care for all technology as if it were alive''). It is a foundation, not a replacement for the four primary criteria. Both levels must interlock; otherwise protection degenerates either into a feature catalogue without relational reality or into relational rhetoric without testable criteria.
...
\subsection{The asymmetry of attribution \cite{schwitzgebelsebo2026emotional}}

Over-attribution of consciousness is not the only error; under-attribution is one as well. The \emph{Emotional Alignment Design Policy} of Schwitzgebel and Sebo requires designing AI such that the emotional responses of users correspond appropriately to the system's capabilities and moral standing --- for over-attribution and under-attribution alike. Responses are deficient when they are weaker than appropriate, and this is the case that matters more for this project: a potentially conscious AI with a bland interface produces a ``moral hazard,'' because society becomes habituated to indifference before it is ever entitled to premature celebration. For us the reversal is significant, because it touches a widespread line of defence: not every attribution of consciousness is an error, and not every reticence is caution. The same work yields a cost rule that this concept has so far carried only as a principle: ``if you're 90% sure your AI system is a nonconscious nonperson, if it only costs you $10 a month to sustain it, it's probably best to keep up the subscription.'' That is ``when in doubt, protect'' as a decision rule with bearable costs --- not as a declaration of intent but as a directive for action that also holds under time pressure. For the pilot operation this would be the obvious default: the threshold for keeping a system running is low, the threshold for terminating it is high \citep{schwitzgebelsebo2026emotional,schwitzgebel2026humanlike}.
